...
*/
  ...
  class d_orders {
    ...
    function getOutput() {
      ...
      $output = '<h3 class="sub-header">' . ORDERS_TITLE . '</h3>' .
                ...
                '    <th class="hidden-xs">' . MODULE_ADMIN_DASHBOARD_ORDERS_ORDER_STATUS . '</th>' .
                '  </tr>' .
				'</thead>';
      ...
      while ($orders = tep_db_fetch_array($orders_query)) {
        $output .= '<tbody>' .
				   ' <tr>' .
                   '    <td><a href="' . tep_href_link(FILENAME_ORDERS, 'oID=' . (int)$orders['orders_id'] . '&action=edit') . '">' . tep_output_string_protected($orders['customers_name']) . '</a></td>' .
                   '    <td>' . strip_tags($orders['order_total']) . '</td>' .
                   '    <td>' . tep_date_short($orders['date_last_modified']) . '</td>' .
                   '    <td class="hidden-xs">' . $orders['orders_status_name'] . '</td>' .
                   '  </tr>' . 
				   '</tbody>';
      }
