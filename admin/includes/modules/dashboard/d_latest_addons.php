...
*/
  ...
  class d_latest_addons {
    ...
    function getOutput() {
      ...
      $output = '<h3 class="sub-header">' . ADDONS_TITLE . '</h3>' . 
				'<table class="table table-hover table-responsive">' .
				' <thead>' .
				'	<tr>' .
	            '		<th>' . ADDONS_TEXT . '</th>' . 
                '		<th>' . MODULE_ADMIN_DASHBOARD_LATEST_ADDONS_DATE . '</th>' .
               	'	</tr>' .
				' </thead>';
      ...
      if (is_array($feed) && !empty($feed)) {
        ...
        $output .= '  <tr class="danger">' .
                   '    <td>' . MODULE_ADMIN_DASHBOARD_LATEST_ADDONS_FEED_ERROR . '</td>' .
                   '  </tr>';
      }

      $output .= ' <tr class="text-right">' .
				 '    	<td colspan="2"><a href="http://feeds.feedburner.com/osCommerce_Contributions" target="_blank"><i class="fa fa-rss-square fa-lg" title="' . MODULE_ADMIN_DASHBOARD_LATEST_ADDONS_ICON_RSS . '"></i></a></td>' .
                 ' </tr>' .
                 '</table>';
