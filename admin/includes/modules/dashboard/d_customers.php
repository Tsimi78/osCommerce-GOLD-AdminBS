...
*/
  ...
  class d_customers {
    ...
    function getOutput() {
      $output = '<h3 class="sub-header">' . CUSTOMERS_TITLE . '</h3>' .
				'<table class="table table-hover table-responsive">' .
                '<thead>' .
				'<tr>' . 
                '    <th>' . MODULE_ADMIN_DASHBOARD_CUSTOMERS_TITLE . '</td>' .
                '    <th>' . MODULE_ADMIN_DASHBOARD_CUSTOMERS_DATE . '</td>' .
                '  </tr>' .
				'</thead>';
      ...
      while ($customers = tep_db_fetch_array($customers_query)) {
        $output .= '<tbody>' .
				   '  <tr>' .
                   '    <td><a href="' . tep_href_link(FILENAME_CUSTOMERS, 'cID=' . (int)$customers['customers_id'] . '&action=edit') . '">' . tep_output_string_protected($customers['customers_firstname'] . ' ' . $customers['customers_lastname']) . '</a></td>' .
                   '    <td>' . tep_date_short($customers['customers_info_date_account_created']) . '</td>' .
                   '  </tr>' . 
				   '</tbody>';
      }
