...
*/
  ...
  class d_admin_logins {
    ...
    function getOutput() {
      $output = '<h3 class="sub-header">' . ADMIN_LOGINS_TITLE . '</h3>' . 
				'<table class="table table-hover table-responsive">' .
				' <thead>' .
				'	<tr>' .
				'		<th>&nbsp;</th>' .
                '		<th>' . TEXT_ADMIN_LOGINS . '</th>' .
                '		<th>' . MODULE_ADMIN_DASHBOARD_ADMIN_LOGINS_DATE . '</th>' .
                '	</tr>' .
				' </thead>';
      ...
      while ($logins = tep_db_fetch_array($logins_query)) {
        $output .= '<tbody>' .
				   '  <tr>' .
				   '	<td>' . (($logins['success'] == '1') ? '<i class="fa fa-check fa-lg icon-green"></i>' : '<i class="fa fa-times fa-lg icon-red"></i>') . '</td>' .
                   '	<td><a href="' . tep_href_link(FILENAME_ACTION_RECORDER, 'module=ar_admin_login&aID=' . (int)$logins['id']) . '">' . tep_output_string_protected($logins['user_name']) . '</a></td>' .
                   '	<td>' . tep_date_short($logins['date_added']) . '</td>' .
                   '  </tr>' . 
				   '</tbody>';
      }
